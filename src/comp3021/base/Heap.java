...
public class Heap<T extends Comparable<T>> {
    ...
    private void heapifyUp() {
        // TODO
        int qqqq = container.size()-1;
        boolean ttt = true;
        while ( ttt  ){
            if (hasParent(qqqq)) {
                //System.out.println(container);
                //System.out.println("Container size:"+container.size());
                //System.out.println("Last element:"+container.get(qqqq));
                //System.out.println("Last:"+container.get(qqqq)+"  Parent:"+container.get(getParentIndex(qqqq)));
                if (container.get(getParentIndex(qqqq)).compareTo(container.get(qqqq)) > 0) {
                    //System.out.println("Last element is smaller than parent, swapping");
                    swap(qqqq,getParentIndex(qqqq));
                    //System.out.println("New Last:"+container.get(qqqq)+"  Parent:"+container.get(getParentIndex(qqqq)));
                } else {
                    ttt = false;
                }
                qqqq = getParentIndex(qqqq);
            }else {
                ttt = false;
            }
            //System.out.println(container);
